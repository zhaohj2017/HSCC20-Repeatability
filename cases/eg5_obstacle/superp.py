...
FRACTION_INSTABLE_BATCH = 10000000000000000000000
NUM_BATCH_ITR = 3
...
BLOCK_LEN_D = np.power(2, BLOCK_EXP_D) # number of batches for each dimension
    ...
    # for this example, it is important to keep the relative sizes of init, unsafe and domain-lie for each batch
...
PLOT_EXP_V = np.array([7, 7, 7]) # sampling from domain for plotting the vector field
PLOT_LEN_V = np.power(2, PLOT_EXP_V) # the number of samples for each dimension of domain, usually equal to superp.DATA_LEN_D

PLOT_EXP_P = np.array([7, 7, 7]) # sampling from domain for plotting the scattering sampling points, should be equal to superp.DATA_LEN_D
