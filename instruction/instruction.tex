...
Suppose you are using Linux or Mac and have changed into the directory \textsf{HSCC20-Repeatability}. 
\subsubsection{Pre-train.} First, execute the command
\begin{verbatim}              
        cp ./cases/eg1_prajna_original/*.py .
\end{verbatim}
Next modify the following 3 lines in \textsf{superp.py} in the current directory
\inputpython{../cases/eg1_prajna_original/superp.py}{33}{35}
to set all the three tolerances to 0.
Then run 
\begin{verbatim}              
        python main.py
\end{verbatim}
to obtain a generated barrier certificate and a pre-trained model which will be stored in the 
\textsf{pre-trained.pt} file in the current directory. At the same time, a picture similar to
the left part of Figure~6 will be produced.

\subsubsection{Fine-tune.}
In the following, modify the {\textsf{superp.py}} file in the 
current directory
\inputpython{../cases/eg1_prajna_original/superp.py}{17}{17}
by setting the \textsf{\small FINE\_TUNE} option to 1. Next 
modify the following 3 lines in \textsf{superp.py} in the current directory
\inputpython{../superp.py}{33}{35}
to set the three tolerances to larger values, say, \pyth{TOL_SAFE = 0.05}. Then run 
\begin{verbatim}              
        python main.py
\end{verbatim}
again to obtain a generated barrier certificate and a fine-tuned model which will be stored in the 
\textsf{pre-trained.pt} file in the current directory. At the same time, a picture similar to
the right part of Figure~6 will be produced.

The above fine-tuning operations can be iteratively performed.
...
are to set the parameters of our designed learning rate adjusting strategy;
Line
